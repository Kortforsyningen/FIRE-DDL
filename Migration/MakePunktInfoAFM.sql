...
DELETE FROM PUNKTINFO WHERE INFOTYPEID IN (SELECT INFOTYPEID FROM PUNKTINFOTYPE WHERE INFOTYPE LIKE 'AFM:%');
...
SELECT
    ...
    (SELECT INFOTYPEID FROM PUNKTINFOTYPE WHERE INFOTYPE = 'AFM:' || afmfrom.AFMTYP AND ROWNUM <= 1) AS INFOTYPEID,
...
SELECT
    afmfrom.IN_DATE AS REGISTRERINGFRA,
    afmto.IN_DATE AS REGISTRERINGTIL,
    (SELECT INFOTYPEID FROM PUNKTINFOTYPE WHERE INFOTYPE = 'AFM:diverse' AND ROWNUM <= 1) AS INFOTYPEID,
    afm.TEKST AS TEKST,
    '15101d43-ac91-4c7c-9e58-c7a0b5367910' AS SAGSEVENTFRAID,
    conv.ID AS PUNKTID
FROM FYSIK_AFM afmfrom
...
SELECT
    afmfrom.IN_DATE AS REGISTRERINGFRA,
    afmto.IN_DATE AS REGISTRERINGTIL,
    (SELECT INFOTYPEID FROM PUNKTINFOTYPE WHERE INFOTYPE = CASE
    ...
    END AND ROWNUM <= 1) AS INFOTYPEID,
    '15101d43-ac91-4c7c-9e58-c7a0b5367910' AS SAGSEVENTFRAID,
    conv.ID AS PUNKTID
FROM FYSIK_AFM afmfrom
INNER JOIN CONV_PUNKT conv ON afmfrom.REFNR = conv.REFNR
...
SELECT
    ...
    (SELECT INFOTYPEID FROM PUNKTINFOTYPE WHERE INFOTYPE = 'AFM:naturlig' AND ROWNUM <= 1) AS INFOTYPEID,
    NULL AS TEKST,
    '15101d43-ac91-4c7c-9e58-c7a0b5367910' AS SAGSEVENTFRAID,
    conv.ID AS PUNKTID
FROM PUNKT p
...
-- AFM:postament, bemærk ingen historik pga. QUICKSTNINFORM (indeholder kun gældende værdier)
INSERT INTO PUNKTINFO (REGISTRERINGFRA, REGISTRERINGTIL, INFOTYPEID, TEKST, SAGSEVENTFRAID, PUNKTID)
SELECT
    href.OPRDATO AS REGISTRERINGFRA,
    NULL AS REGISTRERINGTIL,
    (SELECT INFOTYPEID FROM PUNKTINFOTYPE WHERE INFOTYPE = 'AFM:postament' AND ROWNUM <= 1) AS INFOTYPEID,
    NULL AS TEKST,
    '15101d43-ac91-4c7c-9e58-c7a0b5367910' AS SAGSEVENTFRAID,
    conv.ID AS PUNKTID
FROM PUNKT p
INNER JOIN CONV_PUNKT conv ON p.ID = conv.ID
INNER JOIN HVD_REF@refgeo href ON href.REFNR = conv.REFNR
INNER JOIN QUICKSTNINFORM@refgeo n ON n.REFNR = conv.REFNR AND n.p_mark=1
;
...
INSERT INTO PUNKTINFO (REGISTRERINGFRA, REGISTRERINGTIL, INFOTYPEID, TAL, SAGSEVENTFRAID, PUNKTID)
SELECT
    hfrom.IN_DATE AS REGISTRERINGFRA,
    hto.IN_DATE AS REGISTRERINGTIL,
    (SELECT INFOTYPEID FROM PUNKTINFOTYPE WHERE INFOTYPE = 'AFM:højde_over_' || trim('.' FROM height.TEKST) AND ROWNUM <= 1) AS INFOTYPEID,
    hfrom.HEIGHT AS TAL,
    '15101d43-ac91-4c7c-9e58-c7a0b5367910' AS SAGSEVENTFRAID,
    conv.ID AS PUNKTID
