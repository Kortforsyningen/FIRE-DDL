...
SELECT se.OBJEKTID, rap.RAPPORT
...
-- dvr90 + fvr09
INSERT INTO KOORDINAT (REGISTRERINGFRA, REGISTRERINGTIL, SRIDID, SX, SY, SZ, T, TRANSFORMERET, ARTSKODE, FEJLMELDT, X, Y, Z, SAGSEVENTFRAID, PUNKTID)
SELECT coor.IN_DATE, coor.OUT_DATE, coor.SRIDID, coor.SX, coor.SY, CASE WHEN coor.SZ IS NULL THEN 999 ELSE coor.SZ END, coor.T, CASE WHEN coor.ARTSKODE = 6 THEN 'true' ELSE 'false' END, coor.ARTSKODE, 'false', coor.X, coor.Y, coor.Z, said.SAID, p.ID
...
INSERT INTO KOORDINAT (REGISTRERINGFRA, REGISTRERINGTIL, SRIDID, SX, SY, SZ, T, TRANSFORMERET, ARTSKODE, FEJLMELDT, X, Y, Z, SAGSEVENTFRAID, PUNKTID)
SELECT coor.IN_DATE, coor.OUT_DATE, coor.SRIDID, coor.SX, coor.SY, coor.SZ, coor.T, CASE WHEN coor.ARTSKODE = 6 THEN 'true' ELSE 'false' END, coor.ARTSKODE, 'false', coor.X, coor.Y, coor.Z, said.SAID, p.ID
FROM PUNKT p INNER JOIN CONV_PUNKT conv ON p.ID = conv.ID
INNER JOIN
(
...
-- s34j
INSERT INTO KOORDINAT (REGISTRERINGFRA, REGISTRERINGTIL, SRIDID, SX, SY, SZ, T, TRANSFORMERET, ARTSKODE, FEJLMELDT, X, Y, Z, SAGSEVENTFRAID, PUNKTID)
SELECT coor.IN_DATE, coor.OUT_DATE, coor.SRIDID, CASE WHEN coor.SX IS NULL THEN 999 ELSE coor.sx END, CASE WHEN coor.SY IS NULL THEN 999 ELSE coor.SY END, coor.SZ, coor.T, CASE WHEN coor.ARTSKODE = 6 THEN 'true' ELSE 'false' END, coor.ARTSKODE,'false', coor.X, coor.Y, coor.Z, said.SAID, p.ID
FROM PUNKT p INNER JOIN CONV_PUNKT conv ON p.ID = conv.ID
INNER JOIN
(
