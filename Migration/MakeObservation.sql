...
-- Make lookup table OBSSAGSEVENTID containing every unique insert date for all observation and a new unique ID
-- Will be used to create sagsevents and to populate column OBSERVATION.SAGSEVENTID for all observaton types   
...
SELECT 
  ...
  'geometrisk_koteforskel' AS OBSERVATIONSTYPE,
  opst.SETS AS ANTAL,
...
-- geometrisk_koteforskel 2, magen til ovenfor dog med value8
INSERT INTO OBSERVATION (
  REGISTRERINGFRA,
  REGISTRERINGTIL,
  OBSERVATIONSTIDSPUNKT,
...
SELECT 
  ...
  NVL(obs.OBS_DATE, TO_DATE(opst.YEAR, 'YYYY')) AS OBSERVATIONSTIDSPUNKT,
  obsconv.ID AS SIGTEPUNKTID,
  'trigonometrisk_koteforskel' AS OBSERVATIONSTYPE,
  opst.SETS AS ANTAL,
  grp.GROUPID AS GRUPPE,
  opstconv.ID AS OPSTILLINGSPUNKTID,
  obs.OBS AS VALUE1,
...
SELECT 
  ...
  obsconv.ID AS SIGTEPUNKTID,
  'zenitvinkel' AS OBSERVATIONSTYPE,
  opst.SETS AS ANTAL,
  grp.GROUPID AS GRUPPE,
  opstconv.ID AS OPSTILLINGSPUNKTID,
  obs.OBS AS VALUE1,
...
SELECT 
  ...
  TO_DATE(CASE WHEN opst.YEAR IS NULL OR opst.YEAR < 1700 THEN 1700 ELSE opst.YEAR END, 'YYYY') AS OBSERVATIONSTIDSPUNKT,
  obsconv.ID AS SIGTEPUNKTID,
  'skråafstand' AS OBSERVATIONSTYPE,
  opst.SETS AS ANTAL,
  grp.GROUPID AS GRUPPE,
  opstconv.ID AS OPSTILLINGSPUNKTID,
  obs.OBS AS VALUE1,
...
INNER JOIN CONV_PUNKT obsconv ON obs.REFNR = obsconv.REFNR
INNER JOIN PUNKT obsp ON obsp.ID = obsconv.ID
-- LEFT JOIN GEOMETRIOBJEKT obsgeom ON obsp.ID = obsgeom.PUNKTID
LEFT JOIN OBSSAGSEVENTID sevent ON sevent.IN_DATE = opst.IN_DATE
LEFT JOIN OBSGROUPS2 grp ON opst.O_REFNR = grp.O_REFNR
;    
...
-- Make sagseventinfo based on info from JSBR_OBS
